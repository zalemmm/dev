...
function get_inscription() {
	...
	if ( $_GET['send'] == 'confirm' ) {
		...
		if ($_POST['input_11'] == 'true') { //sprawdzam czy adres doreczenia wystepuje
			$l_name = $_POST['l_name'];
			$l_comp = $_POST['l_comp'];
...
function get_inscription2() {
	...
	function get_acces_panel($p) {
		...
		if ($p == 2) {
			...
			<div class="acces_tab_content"><span id="notregistered"><span class="highlight">Je m\'enregistre pour continuer mon devis et télécharger mes fichiers</span></span><a href="'.get_bloginfo('url').'/inscription/" class="pleaseregister">S\'ENREGISTRER</a></div>
			</div>';
		}
	...
	//connect to database
mysql_connect('127.0.0.1', 'frbanderolecom', 'JidrePai');
mysql_select_db('frbanderolecom');

//get the username
$username = mysql_real_escape_string($_POST['wp_fbs_users']);
