<div id="buying">
		...
		<div class="form-all">
				...
				<li class="form-line select" id="id_support">
					<span class="helpButton">
						...
						<span class="helpText" id="helpTextsupport" style="visibility:hidden;">
							<b>tissu 220g M1</b>: tissu stretch léger 100% polyester 220g traité retardant au feu M1<br />
							<b>tissu 260g M1</b>: tissu extensible 100% polyester 260g traité retardant au feu M1.<br />
						</span>
					</span>
            ...
            <option value="tissu 220g">tissu stretch léger 220g M1</option>
                ...
                <li id="id_14" class="form-line optionsformline2" style="nothing">
					<label class="form-label-left label-highlight" id="label_14" for="input_14">taille<strong><br />PLATEAU</strong></label>
					<input type="text" class="taille form-textbox validate[required, Numeric]" placeholder="longueur" id="input_14" name="q14_taile" size="20" value="1" onclick="JKakemono.czyscpola();" />
					...
					<select class="form-dropdown validate[required] option6" id="input_retombee" name="q24_finition1" onchange="JKakemono.czyscpola(); ">
						<option value="">Retombée</option>
                        <option value="0">Sans retombée</option>
						<option value="10">10cm sur les côtés</option>
                        <option value="20">20cm sur les côtés</option>
                        <option value="30">30cm sur les côtés</option>
                        <option value="40">40cm sur les côtés</option>
                        <option value="50">50cm sur les côtés</option>
                        <option value="60">60cm sur les côtés</option>
                        <option value="70">70cm sur les côtés</option>
                        <option value="80">80cm sur les côtés</option>
                        <option value="90">90cm sur les côtés</option>
				...
				<li id="id_14rond" class="form-line optionsformline2" style="nothing">
					<label class="form-label-left label-highlight" id="label_14rond" for="input_14rond">diamètre<strong><br />PLATEAU</strong></label>
					...
					<span class="mLeft highlight">cm</span>

                    <select class="form-dropdown validate[required] option6" id="input_retombee_rond" name="q24_finition1" onchange="JKakemono.czyscpola(); ">
						<option value="">Retombée</option>
                        <option value="0">Sans retombée</option>
						<option value="10">10cm de retonbée</option>
                        <option value="20">20cm de retonbée</option>
                        <option value="30">30cm de retonbée</option>
                        <option value="40">40cm de retonbée</option>
                        <option value="50">50cm de retonbée</option>
                        <option value="60">60cm de retonbée</option>
                        <option value="70">70cm de retonbée</option>
                        <option value="80">80cm de retonbée</option>
                        <option value="90">90cm de retonbée</option>
	...
	<div id="preview">
	  ...
	  <img src="../wp-content/plugins/fbshop/images/arrow.png" alt="arrow" class="arrow" />
	    ...
	    <div id="slides">
				<li><img src="../wp-content/plugins/fbshop/images/nappe/slide/nappe-publicitaire0.jpg" alt="nappe publicitaire devis en ligne" title="nappe conférence meeting salons meilleur prix" /></li>
	      <li><img src="../wp-content/plugins/fbshop/images/nappe/slide/nappe-publicitaire1.jpg" alt="nappe publicitaire pas cher" title="nappe publicitaire pas cher" /></li>
				<li><img src="../wp-content/plugins/fbshop/images/nappe/slide/nappe-publicitaire2.jpg" alt="nappe publicitaire évènement" title="nappe imprimée" /></li>
				<li><img src="../wp-content/plugins/fbshop/images/banderole/slide/devis-en-ligne.png" alt="commencez votre devis en ligne" title="devis impression nappe publicitaire" /></li>
	    </div>
	  </div>
...
<script type="text/javascript">
	...
	jQuery(document).ready(function(){
		...
		var retombee = 0;
		retombee              = $('input_retombee').value;
		var retombee_rond = 0;
		retombee_rond          = $('input_retombee_rond').value;
		var rajout = retombee/100 ;
		var rajout_rond = retombee_rond/100 ;



		////////////////////////////////////////////////////////////////////////////
		largeursite              = $('input_15').value;
		largeursite               = largeursite.replace(',','.');
		largeursite               = fixstr(largeursite);
		$('input_15').value = largeursite ;
		longueursite                = $('input_14').value;
		longueursite                = longueursite.replace(',','.');
		longueursite                = fixstr(longueursite);
		$('input_14').value	 = longueursite ;

		diametresite                = $('input_14rond').value;
		diametresite                = diametresite.replace(',','.');
		diametresite                = fixstr(diametresite);
		$('input_14rond').value	 = diametresite ;

		diametresite               = parseFloat(diametresite)/100 ;
		largeursite                = parseFloat(largeursite)/100 ;
		longueursite                = parseFloat(longueursite)/100 ;

		diametre = diametresite+(rajout_rond*2);
		largeur = largeursite+(rajout*2);
		longueur = longueursite+(rajout*2);
		...
		if ((largeur >= 1.56) && (largeur <= 2.60)){l1=2.6; l2=2.6-largeur; perteL=l2*longueur;};
		...
		if ((perteH < perteL) && ($('input_forme').value == 'rectangulaire')){
			...
			////tissu 220g
			if (($('input_support').value == 'tissu 220g' ) && (h1<=1.00) ){plm =15.50 ;}
			if (($('input_support').value == 'tissu 220g' ) && ((h1>=1.01)&&(h1<=1.60)) ){plm =16.48 ;}
			if (($('input_support').value == 'tissu 220g' ) && ((h1>=1.61)&&(h1<=2.00))){plm =17.45 ;}
			if (($('input_support').value == 'tissu 220g' ) && ((h1>=2.01)&&(h1<=2.50))){plm =18.43 ;}
			if (($('input_support').value == 'tissu 220g' ) && (h1>=2.51)){plm =19.40 ;}
			////tissu 260g
			if (($('input_support').value == 'tissu 260g' ) && (h1<=1.00) ){plm =25.50 ;}
			if (($('input_support').value == 'tissu 260g' ) && ((h1>=1.01)&&(h1<=1.60)) ){plm =28.08 ;}
			if (($('input_support').value == 'tissu 260g' ) && ((h1>=1.61)&&(h1<=2.00))){plm =29.65 ;}
			if (($('input_support').value == 'tissu 260g' ) && ((h1>=2.01)&&(h1<=2.50))){plm =30.23 ;}
			if (($('input_support').value == 'tissu 260g' ) && (h1>=2.51)){plm =31.80 ;}

			prixproduit = metrage*plm;
			opis += '<br />- forme rectangulaire';
			opis += '<br />- dont' + rajout*100+ 'cm de retombée';
		}
		...
		if($('input_forme').value == 'ronde'){
			...
			prixproduit = metrage*plm;////prix de la banderole
			opis += '<br />- dont' + rajout_rond*100+ 'cm de retombée';
		}
	    ...
	    //////////////////////////////////////////////////////////////////////////
			...
			//----------------------------------------------------------------- + de 50m
			if ((longueur >= 50) && ($('input_forme').value == 'rectangulaire')) {
				eBox.innerHTML = '<button class="closeButton"><i class="ion-ios-close-empty" aria-hidden="true"></i></button><img src="//www.france-banderole.com/wp-content/themes/fb/images/exclamation-octagon.png" class="exclam" alt="attention" /> la longeur ne doit pas dépasser 50m';
				eBox.style.display="block";
				erreurType=1;
			}

			//----------------------------------------------------------------- + de 2.59m
			if ((largeur >= 2.59) && ($('input_forme').value == 'rectangulaire')) {
				eBox.innerHTML = '<button class="closeButton"><i class="ion-ios-close-empty" aria-hidden="true"></i></button><img src="//www.france-banderole.com/wp-content/themes/fb/images/exclamation-octagon.png" class="exclam" alt="attention" /> la largeur ne doit pas dépasser 2,59m!';
				eBox.style.display="block";
				erreurType=1;
			}
			//----------------------------------------------------------------- + de 2.59m
			if ($('input_forme').value == 'ronde'){
			if ((diametre <= 0.49) ||  (diametre >= 2.59)){
			...
			//-----------------------------------------------------------------------nul
			if ( ((largeur <= 0 ) || (longueur <= 0 )) && ($('input_forme').value == 'rectangulaire')){
			...
			if ((erreurType==0) && ((DeliveryType == '2-3') || (DeliveryType == '1-1') || (DeliveryType == '3-4'))) {
				console.log(largeur+'x'+longueur);
				suma=cena-rabat;
				suma=fixstr(suma);
				suma2 = suma.replace(".", ",");
				...
				if($('input_forme').value == 'rectangulaire'){
			      ...
			      saveBlobAsFile(blob, "XX.png").then(
				    data => {
				      base64 = data;
				      sendData();
				    });
			    ...
			    function saveBlobAsFile(blob, fileName) {
					...
					// largeur et longueur x100 pour un affichage en cm côté admin
					dodajkoszyk.innerHTML = '<input type="hidden" name="addtocart" value="addtocart" /><input type="hidden" name="rodzaj" value="'+rodzaj+'" /><input type="hidden" name="opis" value="'+bacheType+'</br>- H|'+(longueur*100).toFixed(0)+' x L|'+(largeur*100).toFixed(0)+' cm'+opis+maquette+optliv+prliv+etiqdesc+'" /><input type="hidden" name="ilosc" value="'+ilosc+'" /><input type="hidden" name="prix" value="'+cena2+' &euro;" /><input type="hidden" name="option" value="'+option2+'" /><input type="hidden" name="remise" value="'+rabat2+'" /><input type="hidden" name="total" value="'+suma2+' &euro;" /><input type="hidden" name="largeur" value="'+(longueur*100).toFixed(0)+'" /><input type="hidden" name="hauteur" value="'+(largeur*100).toFixed(0)+'" /><button id="submit_cart" type="submit"><i class="fa fa-shopping-cart" aria-hidden="true"></i> ajouter au panier</button> ';
					livraisonComp.style.display = 'block';
				}


				// ------------------------------------------------envoi formulaire rond
				if($('input_forme').value == 'ronde'){
					var dodajkoszyk = document.getElementById("cart_form");
					var base64;
					...
					// largeur et longueur x100 pour un affichage en cm côté admin
					dodajkoszyk.innerHTML = '<input type="hidden" name="addtocart" value="addtocart" /><input type="hidden" name="rodzaj" value="'+rodzaj+'" /><input type="hidden" name="opis" value="'+bacheType+'</br>- &empty;|'+(diametre*100).toFixed(0)+' cm'+opis+maquette+optliv+prliv+etiqdesc+'" /><input type="hidden" name="ilosc" value="'+ilosc+'" /><input type="hidden" name="prix" value="'+cena2+' &euro;" /><input type="hidden" name="option" value="'+option2+'" /><input type="hidden" name="remise" value="'+rabat2+'" /><input type="hidden" name="total" value="'+suma2+' &euro;" /><input type="hidden" name="hauteur" value="'+longueur*100+'" /><input type="hidden" name="largeur" value="'+largeur*100+'" /><button id="submit_cart" type="submit"><i class="fa fa-shopping-cart" aria-hidden="true"></i> ajouter au panier</button> ';
					livraisonComp.style.display = 'block';
				}
			}
