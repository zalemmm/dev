...
(function($) {
    $.fn.makeAsyncUploader = function(options) {
        return this.each(function() {
            ...
            container.append($("<span id='" + id + "_uploading'>Transfert du fichier... <input type='button' value='Cancel'/></span>"));
		    ...
		    jQuery("#btnSubmit").css("display", "none");


            // Instantiate the uploader SWF
            var swfu;
            var width = 86, height = 19;
            if (options) {
            ...
            var defaults = {
                ...
                upload_success_handler: function(file, response) {
                    ...
                    $("span[id$=_completedMessage]", container).html("Fichier transféré <b>{0}</b> ({1} KB)"
                                .replace("{0}", file.name)
