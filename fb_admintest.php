...
function fbadm_invoice_print($number) {
	...
	if ($query) {
			...
			//rem banderole
			$exist_remise_b = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_b' AND uid = '$uid'");
			$client_remise_b = $exist_remise_b->att_value;
			//rem roll up
			$exist_remise_r = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_r' AND uid = '$uid'");
				$client_remise_r = $exist_remise_r->att_value;
			//rem Totem
			$exist_remise_t = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_t' AND uid = '$uid'");
				$client_remise_t = $exist_remise_t->att_value;
			//rem stickers
			$exist_remise_st = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_st' AND uid = '$uid'");
				$client_remise_st = $exist_remise_st->att_value;
			//rem oriflamme
			$exist_remise_o = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_o' AND uid = '$uid'");
				$client_remise_o = $exist_remise_o->att_value;
			//enseigne
			$exist_remise_e = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_e' AND uid = '$uid'");
				$client_remise_e = $exist_remise_e->att_value;	
			//akilux
			$exist_remise_a = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_a' AND uid = '$uid'");
				$client_remise_a = $exist_remise_a->att_value;
			//PLV intérieur
			$exist_remise_pi = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_pi' AND uid = '$uid'");
			$client_remise_pi = $exist_remise_pi->att_value;
			//PLV extérieur
			$exist_remise_pe = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_pe' AND uid = '$uid'");
			$client_remise_pe = $exist_remise_pe->att_value;
			//rem stand tissu
			$exist_remise_s = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_s' AND uid = '$uid'");
			$client_remise_s = $exist_remise_s->att_value;
			//rem flyers
			$exist_remise_f = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_f' AND uid = '$uid'");
			$client_remise_f = $exist_remise_f->att_value;
			//rem affiches
			$exist_remise_af = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_af' AND uid = '$uid'");
			$client_remise_af = $exist_remise_af->att_value;
			//rem carte visite
			$exist_remise_v = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_v' AND uid = '$uid'");
			$client_remise_v = $exist_remise_v->att_value;
			//rem dépliants
			$exist_remise_d = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_d' AND uid = '$uid'");
			$client_remise_d = $exist_remise_d->att_value;
			
			
			//$licznik++;
			
			if ($nomproduit == 'banderole') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_b/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_b = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_b = $remise;
				$premise_b = $remiseindividuelle;
							}
			elseif ($nomproduit == 'Roll-up') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_r/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_r = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_r = $remise;
				$premise_r = $remiseindividuelle;
				
			}
			elseif ($nomproduit == 'Totem') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_t/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_t = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_t = $remise;
				$premise_t = $remiseindividuelle;
			}
			elseif ($nomproduit == 'Vinyles Stickers') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_st/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_st = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_st = $remise;
				$premise_st = $remiseindividuelle;
			}
			elseif ($nomproduit == 'Oriflamme') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_o/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_o = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_o = $remise;
				$premise_o = $remiseindividuelle;
			}
			elseif ($nomproduit == 'forex 3mm') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_e/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_ea = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_ea = $remise;
				$premise_ea = $remiseindividuelle;
			}
			elseif ($nomproduit == 'forex 5mm') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_e/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_eb = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_eb = $remise;
				$premise_eb = $remiseindividuelle;
			}
			elseif ($nomproduit == 'Dibond') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_e/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_ec = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_ec = $remise;
				$premise_ec = $remiseindividuelle;
			}
			elseif ($nomproduit == 'Akilux 3mm') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_a/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_aa = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_aa = $remise;
				$premise_aa = $remiseindividuelle;
			}
			elseif ($nomproduit == 'Akilux 3,5mm') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_a/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_ab = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_ab = $remise;
				$premise_ab = $remiseindividuelle;
			}
			elseif ($nomproduit == 'Akilux 5mm') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_a/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_ac = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_ac = $remise;
				$premise_ac = $remiseindividuelle;
			}
			elseif ($nomproduit == 'PVC 300 microns') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_a/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_ad = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_ad = $remise;
				$premise_ad = $remiseindividuelle;
			}
			//plv intérieur
			elseif ($nomproduit == 'CADRE CLIPPANT A4') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pi/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pia = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pia = $remise;
				$premise_pia = $remiseindividuelle;
			}
			elseif ($nomproduit == 'CADRE CLIPPANT A3') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pi/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pib = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pib = $remise;
				$premise_pib = $remiseindividuelle;
			}
			elseif ($nomproduit == 'CADRE CLIPPANT A2') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pi/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pic = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pic = $remise;
				$premise_pic = $remiseindividuelle;
			}
			elseif ($nomproduit == 'CADRE CLIPPANT A1') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pi/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pid = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pid = $remise;
				$premise_pid = $remiseindividuelle;
			}
			elseif ($nomproduit == 'CADRE CLIPPANT A0') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pi/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pie = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pie = $remise;
				$premise_pie = $remiseindividuelle;
			}
			elseif ($nomproduit == 'PHOTOCALL 200x240') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pi/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				...
				$totalproduit = round($totalproduit,2);
				$remise_pif = $remise;
				$premise_pif = $remiseindividuelle;
			}
			elseif ($nomproduit == 'PRÉSENTOIR A4') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pi/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pig = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pig = $remise;
				$premise_pig = $remiseindividuelle;
			}
			elseif ($nomproduit == 'PRÉSENTOIR A3') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pi/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pih = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pih = $remise;
				$premise_pih = $remiseindividuelle;
			}
			elseif ($nomproduit == '10 MINI TENDEURS') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pi/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pii = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pii = $remise;
				$premise_pii = $remiseindividuelle;
			}
			elseif ($nomproduit == '10 TENDEURS 60 cm') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pi/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pij = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pij = $remise;
				$premise_pij = $remiseindividuelle;
			}
			elseif ($nomproduit == 'PORTE DOCUMENT 4P') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pi/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pik = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pik = $remise;
				$premise_pik = $remiseindividuelle;
			}
			elseif ($nomproduit == 'PORTE DOCUMENT 8P') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pi/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pil = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pil = $remise;
				$premise_pil = $remiseindividuelle;
			}	
			//plv extérieur
			elseif ($nomproduit == 'Barrière délimitation') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pe/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pea = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pea = $remise;
				$premise_pea = $remiseindividuelle;
			}
			elseif ($nomproduit == 'Kit de Barrière supplémentaire') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pe/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_peb = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_peb = $remise;
				$premise_peb = $remiseindividuelle;
			}
			elseif ($nomproduit == 'Barrière delimitation 100cm') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pe/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pec = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pec = $remise;
				$premise_pec = $remiseindividuelle;
			}	
			elseif ($nomproduit == 'Cadre extérieur 100x250cm') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pe/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_ped = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_ped = $remise;
				$premise_ped = $remiseindividuelle;
			}
			elseif ($nomproduit == 'Cadre extérieur 125x300cm') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pe/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pee = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pee = $remise;
				$premise_pee = $remiseindividuelle;
			}
			elseif ($nomproduit == 'Stoppeur – chevalet extérieur A1') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pe/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pef = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pef = $remise;
				$premise_pef = $remiseindividuelle;
			}
			elseif ($nomproduit == 'Stoppeur – chevalet extérieur A0') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pe/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_peg = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_peg = $remise;
				$premise_peg = $remiseindividuelle;
			}	
			elseif ($nomproduit == 'Stoppeur extérieur sur ressorts A1') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pe/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_peh = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_peh = $remise;
				$premise_peh = $remiseindividuelle;
			}
			elseif ($nomproduit == 'Stoppeur extérieur sur ressorts A0') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pe/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pei = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pei = $remise;
				$premise_pei = $remiseindividuelle;
			}
			elseif ($nomproduit == 'Potences de suspensions') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pe/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pej = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pej = $remise;
				$premise_pej = $remiseindividuelle;
			}
			elseif ($nomproduit == 'Stand Parapluie') {
				$remisep = $client_remise/100;
				$remiser = $client_remise_pe/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_pek = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_pek = $remise;
				$premise_pek = $remiseindividuelle;
			}
			elseif (strpos($nomproduit, 'Flyer') !== false) {
				$remisep = $client_remise/100;
				$remiser = $client_remise_f/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_f = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_f = $remise;
				$premise_f = $remiseindividuelle;
			}
			elseif (strpos($nomproduit, 'Affiche') !== false) {
				$remisep = $client_remise/100;
				$remiser = $client_remise_af/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_af = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_af = $remise;
				$premise_af = $remiseindividuelle;
			}
			elseif (strpos($nomproduit, 'Cartes') !== false) {
				$remisep = $client_remise/100;
				$remiser = $client_remise_v/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_v = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_v = $remise;
				$premise_v = $remiseindividuelle;
			}
			elseif (strpos($nomproduit, 'depliants') !== false) {
				$remisep = $client_remise/100;
				$remiser = $client_remise_d/100;
				$remisep = round($remisep,2);
				$avantremise = $item[total];
				$avantremise_d = $item[total];
				$remise = $item[total] * $remisep;
				$remiseindividuelle = $item[total] * $remiser;
				$totalproduit = $item[total] - $remise - $remiseindividuelle;
				$totalproduit = round($totalproduit,2);
				$remise_d = $remise;
				$premise_d = $remiseindividuelle;
			}
			else {
			...
			$remiseproduit = str_replace('.', ',', number_format($remiseproduit, 2));
			$remisenormale = $item[total];
			$remise = number_format($remise, 2, ',', ' ');
			$avantremise = number_format($avantremise, 2, ',', ' ');
			$remiseindividuelle = number_format($remiseindividuelle, 2, ',', ' ');
			$totalproduit = number_format($totalproduit, 2, ',', ' ');
			$prixquantite = ($item[ilosc] * $item[prix]);
			$somremiseclient += $remiseindividuelle;
			$somremiseclient = number_format($somremiseclient, 2, ',', ' ');
			$somremiseproduit += $remise;
			$somremiseproduit = number_format($somremiseproduit, 2, ',', ' ');
			
			$remdegr = $item[remise];
			$totalligne = ($item[total]-$remise);
			$ligne_remise = number_format((($item[quantity]*$item[prix])-($remiseindividuelle + $remise)), 2, ',', ' ');
			
				$view .= '<tr><td class="lefttd"><span class="name">'.$item[name].'</span><br /><span class="therest">'.$item[description].'</span></td><td>'.$item[quantity].'</td><td>'.$item[prix].'</td><td>'.$item[prix_option].'</td><td>'.$remise_d.'</td><td>'.number_format(($remiseindividuelle + $remise), 2, ',', ' ').'&nbsp;&euro;</td><td>'.$ligne_remise.'&nbsp;&euro;</td></tr>';
	  		}
...
//sprawdzanie czy jest rabat dla uzytkownika//
			...
			$somremdeg += $ligne_remise; 
			
			$somremiseclient += $ligne_remise;
			...
			if ($exist_remise) {
		  		...
		  		$cremisetd = '<tr><td class="left">REMISE CLIENT</td><td>'.$somremiseclient.'&nbsp;&euro;</td></tr>';
			}
//koniec//  
...
function fb_admin_reports_users() {
	...
	echo '</select>';
	echo '</select> OR <select name="client_color">
	<option value="">Select by color</option>';
		echo '<option value="d3ffde" style="background-color:#d3ffde;display:block;)">#d3ffde</option>';
		echo '<option value="fbcfd0" style="background-color:#fbcfd0;display:block;)">#fbcfd0</option>';
		echo '<option value="d3ccff" style="background-color:#d3ccff;display:block;)">#d3ffd</option>';
		echo '<option value="eeffbb" style="background-color:#eeffbb;display:block;)">#eeffbb</option>';
		echo '<option value="ffe7bb" style="background-color:#ffe7bb;display:block;)">#ffe7bb</option>';
		echo '<option value="fbcff0" style="background-color:#fbcff0;display:block;)">#fbcff0</option>';
		echo '<option value="c9f8fe" style="background-color:#c9f8fe;display:block;)">#c9f8fe</option>';
		echo '<option value="d1ffb4" style="background-color:#d1ffb4;display:block;)">#d1ffb4</option>';
		echo '<option value="e1ceb0" style="background-color:#e1ceb0;display:block;)">#e1ceb0</option>';
		echo '<option value="a6a6a6" style="background-color:#a6a6a6;display:block;)">#a6a6a6</option>';
		echo '<option value="none" style="background:none;">None</option>';
	echo '</select>';
...
$liczbezmak = 0;
 if (isset($_POST['pokaztab']) && ($_POST['users_sales'] == '')) {
	...
	if (isset($_POST['client_color']) && $_POST['client_color']!='') {
		$where = $_POST['client_color'];
		$results = $wpdb->get_results("SELECT * FROM wp_fbs_users_cf WHERE att_name = 'client_color' AND att_value = '".$where."' AND uid != 0");

	echo '<div id="example-1" class="beautifulData fhhhide" style="clear: both;">
			<table>
			<thead>
			<tr>
			<th>Lp.</th>
		...
		foreach ($results as $result) {
			...
			$userinfo = $wpdb->get_row("SELECT * FROM `$fb_tablename_users` WHERE id = ".$id."");
						$exist_color = $wpdb->get_var("SELECT att_value FROM `$fb_tablename_users_cf` WHERE att_name = 'client_color' AND uid = '$id'");
						$style = '';
						if (!empty($exist_color)) {
							$style = ' style="background:#'.$exist_color.'"';
						}
						echo '<tr><td><span '.$style.'> </td><td><span '.$style.'>'.$id.'</span></td><td><span '.$style.'>'.$userinfo->login.'</span></td><td><span '.$style.'>'.$userinfo->f_adress.'</span></td><td><span '.$style.'>'.$userinfo->f_name.'</span></td><td><span '.$style.'>'.$userinfo->f_comp.'</span></td><td><span '.$style.'>'.$userinfo->f_phone.'</span></td><td><span '.$style.'> </span></td><td><form name="us_'.$d->user.'" action="" method="post"><input type="hidden" name="user_name" value="'.$id.'" /><input type="hidden" name="pokaztab" value="true" /><input type="submit" value="View details" class="edit" /></form></td></tr>';
	...
	if (!empty($_POST['checkuserfhh']))
	{	
	$iduser = $_POST['useridfhh'];
	//req sur user
		$s = $wpdb->get_row("SELECT id, login, email, f_name, f_comp, f_phone FROM `$fb_tablename_users` WHERE id = ".$iduser."");
			
		
		if($s) {
		
			echo '
			<style>#example-1 table {display:none;}#example-1 table#singlefh {display:block;}</style>
			<table id="singlefh">
			<thead>
			<tr>
			<th>Lp.</th>
	...
	$sumtotalttc = 0;
 	...
 	if (!empty($_POST['client_saveremise']) && $_POST['client_saveremise'] == 'true'){
		...
		$id_client = $_POST['user_name'];
		$exist_type = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_type' AND uid = ".$id_client."");
		if ($exist_type) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_type' WHERE  att_name = 'client_type' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_type', '$client_type')");
		}
		$client_remise = $_POST['client_remise'];
		$exist_remise = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise' AND uid = ".$id_client."");
		if ($exist_remise) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise' WHERE  att_name = 'client_remise' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise', '$client_remise')");
		}
		$client_remise_b = $_POST['client_remise_b'];
		$exist_remise_b = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_b' AND uid = ".$id_client."");
		if ($exist_remise_b) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise_b' WHERE  att_name = 'client_remise_b' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise_b', '$client_remise_b')");
		}
		
		$client_remise_r = $_POST['client_remise_r'];
		$exist_remise_r = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_r' AND uid = ".$id_client."");
		if ($exist_remise_r) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise_r' WHERE  att_name = 'client_remise_r' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise_r', '$client_remise_r')");
		}
		
		$client_remise_t = $_POST['client_remise_t'];
		$exist_remise_t = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_t' AND uid = ".$id_client."");
		if ($exist_remise_t) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise_t' WHERE  att_name = 'client_remise_t' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise_t', '$client_remise_t')");
		}
		
		$client_remise_o = $_POST['client_remise_o'];
		$exist_remise_o = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_o' AND uid = ".$id_client."");
		if ($exist_remise_o) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise_o' WHERE  att_name = 'client_remise_o' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise_o', '$client_remise_o')");
		}
		
		$client_remise_a = $_POST['client_remise_a'];
		$exist_remise_a = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_a' AND uid = ".$id_client."");
		if ($exist_remise_a) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise_a' WHERE  att_name = 'client_remise_a' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise_a', '$client_remise_a')");
		}
		
		$client_remise_e = $_POST['client_remise_e'];
		$exist_remise_e = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_e' AND uid = ".$id_client."");
		if ($exist_remise_e) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise_e' WHERE  att_name = 'client_remise_e' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise_e', '$client_remise_e')");
		}
		
		$client_remise_pi = $_POST['client_remise_pi'];
		$exist_remise_pi = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_pi' AND uid = ".$id_client."");
		if ($exist_remise_pi) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise_pi' WHERE  att_name = 'client_remise_pi' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise_pi', '$client_remise_pi')");
		}
		
		$client_remise_pe = $_POST['client_remise_pe'];
		$exist_remise_pe = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_pe' AND uid = ".$id_client."");
		if ($exist_remise_pe) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise_pe' WHERE  att_name = 'client_remise_pe' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise_pe', '$client_remise_pe')");
		}
		
		$client_remise_s = $_POST['client_remise_s'];
		$exist_remise_s = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_s' AND uid = ".$id_client."");
		if ($exist_remise_s) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise_s' WHERE  att_name = 'client_remise_s' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise_s', '$client_remise_s')");
		}
		
		$client_remise_te = $_POST['client_remise_te'];
		$exist_remise_te = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_te' AND uid = ".$id_client."");
		if ($exist_remise_te) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise_te' WHERE  att_name = 'client_remise_te' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise_te', '$client_remise_te')");
		}
		
		$client_remise_st = $_POST['client_remise_st'];
		$exist_remise_st = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_st' AND uid = ".$id_client."");
		if ($exist_remise_st) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise_st' WHERE  att_name = 'client_remise_st' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise_st', '$client_remise_st')");
		}
		
		$client_remise_f = $_POST['client_remise_f'];
		$exist_remise_f = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_f' AND uid = ".$id_client."");
		if ($exist_remise_f) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise_f' WHERE  att_name = 'client_remise_f' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise_f', '$client_remise_f')");
		}
		
		$client_remise_af = $_POST['client_remise_af'];
		$exist_remise_af = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_af' AND uid = ".$id_client."");
		if ($exist_remise_af) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise_af' WHERE  att_name = 'client_remise_af' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise_af', '$client_remise_af')");
		}
		
		$client_remise_v = $_POST['client_remise_v'];
		$exist_remise_v = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_v' AND uid = ".$id_client."");
		if ($exist_remise_v) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise_v' WHERE  att_name = 'client_remise_v' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise_v', '$client_remise_v')");
		}
		
		$client_remise_d = $_POST['client_remise_d'];
		$exist_remise_d = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_d' AND uid = ".$id_client."");
		if ($exist_remise_d) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise_d' WHERE  att_name = 'client_remise_d' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise_d', '$client_remise_d')");
		}
		...
		$exist_color = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_color' AND uid = ".$id_client."");
		if ($exist_color) {
			$apdejt = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_color' WHERE  att_name = 'client_color' AND uid = ".$id_client."");
		} else {
			$add_col = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_color', '$client_color')");
		}
 	}
	...
	if ($userinfo) {
		...
		$exist_remise_b = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_b' AND uid = ".$where."");
		$rb = '';
		
		if (!empty($exist_remise_b->att_value)) {
			$rb = $exist_remise_b->att_value;
		}
		
		$exist_remise_r = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_r' AND uid = ".$where."");
		$rr = '';
		
		if (!empty($exist_remise_r->att_value)) {
			$rr = $exist_remise_r->att_value;
		}
		
		$exist_remise_t = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_t' AND uid = ".$where."");
		$rt = '';
		
		if (!empty($exist_remise_t->att_value)) {
			$rt = $exist_remise_t->att_value;
		}
		
		$exist_remise_o = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_o' AND uid = ".$where."");
		$ro = '';
		
		if (!empty($exist_remise_o->att_value)) {
			$ro = $exist_remise_o->att_value;
		}
		
		$exist_remise_a = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_a' AND uid = ".$where."");
		$ra = '';
		
		if (!empty($exist_remise_a->att_value)) {
			$ra = $exist_remise_a->att_value;
		}
		
		$exist_remise_e = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_e' AND uid = ".$where."");
		$re = '';
		
		if (!empty($exist_remise_e->att_value)) {
			$re = $exist_remise_e->att_value;
		}
		
		$exist_remise_pi = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_pi' AND uid = ".$where."");
		$rpi = '';
		
		if (!empty($exist_remise_pi->att_value)) {
			$rpi = $exist_remise_pi->att_value;
		}
		
		$exist_remise_pe = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_pe' AND uid = ".$where."");
		$rpe = '';
		
		if (!empty($exist_remise_pe->att_value)) {
			$rpe = $exist_remise_pe->att_value;
		}
		
		$exist_remise_s = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_s' AND uid = ".$where."");
		$rs = '';
		
		if (!empty($exist_remise_s->att_value)) {
			$rs = $exist_remise_s->att_value;
		}
		
		$exist_remise_st = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_st' AND uid = ".$where."");
		$rst = '';
		
		if (!empty($exist_remise_st->att_value)) {
			$rst = $exist_remise_st->att_value;
		}
		
		$exist_remise_f = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_f' AND uid = ".$where."");
		$rf = '';
		
		if (!empty($exist_remise_f->att_value)) {
			$rf = $exist_remise_f->att_value;
		}
		
		$exist_remise_af = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_af' AND uid = ".$where."");
		$raf = '';
		
		if (!empty($exist_remise_af->att_value)) {
			$raf = $exist_remise_af->att_value;
		}
		
		$exist_remise_v = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_v' AND uid = ".$where."");
		$rv = '';
		
		if (!empty($exist_remise_v->att_value)) {
			$rv = $exist_remise_v->att_value;
		}
		
		$exist_remise_d = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_d' AND uid = ".$where."");
		$rd = '';
		
		if (!empty($exist_remise_d->att_value)) {
			$rd = $exist_remise_d->att_value;
		}
		...
		if (!empty($exist_color->att_value)) {
			...
			if ($exist_color->att_value == 'none') $c11 = ' checked="checked"';
			if ($exist_color->att_value == 'a6a6a6') $c12 = ' checked="checked"';
		}
		...
		echo '<div style="float:left;display:inline;padding:5px;width:30%;height:450px;border:1px solid #ccc;margin:10px 0 10px 10px;font-size:11px;line-height:14px;">
		...
		<div style="float:left;display:block;width:30px;height:20px;line-height:20px;"><input type="radio" name="client_color" value="none"'.$c11.' /> <span style="display:block;width:20px;height:20px;margin:5px 0 0 0;background:none;"></span></div>
		<div style="float:left;display:block;width:30px;height:20px;line-height:20px;"><input type="radio" name="client_color" value="a6a6a6"'.$c12.' /> <span style="display:block;width:20px;height:20px;margin:5px 0 0 0;background:#a6a6a6;"></span></div>
		<br /><br /><br /><br />
		<style>
		.rempo {display:inline-block;margin-left:0px;width:50px}
		label.remnom {display:inline-block;width:120px;}
		</style>
		<div style="float:left;width:45%">
		<label class="remnom"><b>Banderole:</b></label> <input type="text" class="rempo" name="client_remise_b" value="'.$rb.'" size="4" /> %<br/>
		<label class="remnom"><b>Roll up:</b></label> <input type="text" class="rempo" name="client_remise_r" value="'.$rr.'" size="4" /> %<br />
		<label class="remnom"><b>Totem:</b></label> <input type="text" class="rempo" name="client_remise_t" value="'.$rt.'" size="4" /> %<br />
		<label class="remnom"><b>Oriflamme:</b></label> <input type="text" class="rempo" name="client_remise_o" value="'.$ro.'" size="4" /> %<br />
		<label class="remnom"><b>Akilux:</b></label> <input type="text" class="rempo" name="client_remise_a" value="'.$ra.'" size="4" /> %<br />
		<label class="remnom"><b>Enseigne:</b></label> <input type="text" class="rempo" name="client_remise_e" value="'.$re.'" size="4" /> %<br />
		<label class="remnom"><b>Plv int:</b></label> <input type="text" class="rempo" name="client_remise_pi" value="'.$rpi.'" size="4" /> %<br />
		<label class="remnom"><b>Plv ext:</b></label> <input type="text" class="rempo" name="client_remise_pe" value="'.$rpe.'" size="4" /> %<br />
		</div>
		<div style="float:left;width:45%">
		<label class="remnom"><b>Stand tissu:</b></label> <input type="text" class="rempo" name="client_remise_s" value="'.$rs.'" size="4" /> %<br />
		<label class="remnom"><b>Stickers:</b></label> <input type="text" class="rempo" name="client_remise_st" value="'.$rst.'" size="4" /> %<br />
		<label class="remnom"><b>Flyers:</b></label> <input type="text" class="rempo" name="client_remise_f" value="'.$rf.'" size="4" /> %<br />
		<label class="remnom"><b>Affiches:</b></label> <input type="text" class="rempo" name="client_remise_af" value="'.$raf.'" size="4" /> %<br />
		<label class="remnom"><b>C visite:</b></label> <input type="text" class="rempo" name="client_remise_v" value="'.$rv.'" size="4" /> %<br />
		<label class="remnom"><b>Dépliant:</b></label> <input type="text" class="rempo" name="client_remise_d" value="'.$rd.'" size="4" /> %<br />
		</div>
		<br/>
		<br/>
		<br/>
		<input type="submit" value="save changes" /><input type="hidden" name="client_saveremise" value="true" /><input type="hidden" name="pokaztab" value="" /><input type="hidden" name="user_name" value="'.$_POST["user_name"].'" /><input type="hidden" name="useridfhh" value="'.$userinfo->id.'" /><input type="hidden" name="checkuserfhh" value="testok" /><input type="hidden" name="user_login" value="'.$_POST["user_login"].'" />
		</form>
		</div>';
	}	
 ...
 if (isset($_POST['pokaztab']) && ($_POST['users_sales'] == '')) {
	...
	$sumtotalttc = 0;
 	...
 	if (!empty($_POST['client_saveremise']) && $_POST['client_saveremise'] == 'true'){
		...
		$exist_remise_b = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_remise_b' AND uid = ".$id_client."");
		if ($exist_remise_b) {
			$apdejtb = $wpdb->query("UPDATE `$fb_tablename_users_cf` SET att_value='$client_remise_b' WHERE  att_name = 'client_remise_b' AND uid = ".$id_client."");
		} else {
			$add_colb = $wpdb->query("INSERT INTO `$fb_tablename_users_cf` VALUES (not null, '$id_client', 'client_remise_b', '$client_remise_b')");
		}
		$client_color = $_POST['client_color'];
		$exist_color = $wpdb->get_row("SELECT * FROM `$fb_tablename_users_cf` WHERE att_name = 'client_color' AND uid = ".$id_client."");
		if ($exist_color) {
	...
	$userinfo = $wpdb->get_row("SELECT * FROM `$fb_tablename_users` WHERE id = ".$id_client."");
	if ($userinfo) {
		$explode = explode('|', $userinfo->f_address);
		$f_address = $explode['0'];
		$f_porte = $explode['1'];
...
//				$cremisetd = '<tr><td class="toleft">REMISE ('.$.'%)</td><td class="toright">'.$wysokoscrabatu.' &euro;</td></tr>';
				echo '<tr><td colspan="6"></td><td colspan="2" style="text-align:right">REMISE CLIENT('.$exist_remise->percent.'%):</td><td>'.str_replace('.', ',', $wysokoscrabatu).' &euro;</td></tr>';	
			}
//koniec//  		
