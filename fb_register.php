...
function displayAd($address, $edit, $del, $label, $check) {
	...
	$view = '<div class="blocAdresse blocA5">
		...
		'.$address->l_city.'<br />
		'.$address->l_phone.'<br />
		<div class="blocAdresseSelect">envoyer à cette adresse:

			<label class="switch"><input type="radio" name="adresse" form="adresseCheck" value="'.$check.'" class="checkbox" /><span class="slider round"></span></label>
...
function get_inscription2() {
	...
	if (fb_is_logged() && ($isOwner->user == $_SESSION['loggeduser']->id)) {
		...
		<div class="acces_tab_content2">
			...
			<div class="adresses">
			  <input type="hidden" name="adresseSelect" value="select" form="adresseCheck" /><input type="hidden" name="orderid" value="'.$goback.'" form="adresseCheck" />';
		...
		if($user) {
			...
			<div class="blocAdresse blocSelect">
				...
				'.$user->l_city.'<br />
				'.$user->l_phone.'<br />
				<div class="blocAdresseSelect headSelect">adresse actuelle

						<label class="switch"><input type="radio" name="adresse" form="adresseCheck" value="adresse0" class="checkbox" checked /><span class="slider round"></span>
						</label>
			...
			if(isset($_POST['editf'])) {
				...
				<div class="blocAdresse">
					...
					<div class="blocAdresseSelect">envoyer à l\'adresse de facturation:

							<label class="switch"><input type="radio" name="adresse" form="adresseCheck" value="adresseFact" class="checkbox" /><span class="slider round"></span></label>
			...
			if(isset($_POST['editf'])) {
				...
				<div class="blocAdresse blocSelect">
					...
					'.$usertable->f_city.'<br />
					'.$usertable->f_phone.'<br />
					<div class="blocAdresseSelect headSelect">livré à l\'adresse de facturation

						<label class="switch"><input type="radio" name="adresse" form="adresseCheck" value="adresseFact" class="checkbox" checked /><span class="slider round"></span></label>
			...
			if(isset($_POST['addNew'])) {
				...
				<div class="blocAdresse">
					<form name="addLivr" id="addLiv" action="" method="post"></form>
						...
						<label class="cyan">Intitulé (facultatif):
...
function get_acces_panel($p) {
	...
	if ($p == 2) {
		...
		<div class="acces_tab_content"><span id="notregistered">L\'inscription vous permet d\'enregister vos devis, votre panier et de gérer vos commandes (communiquer avec nous, créer vos maquettes en ligne, envoyer vos fichiers etc.) :</span><a href="'.get_bloginfo('url').'/inscription/" class="pleaseregister">S\'enregistrer</a></div>
		</div>';
	}
