	    ...
	    while(($file = readdir($dir))) {      
			if(!is_dir($file) && !in_array($file, array(".",".."))) {
				...
				if($cur_img != 0) {
					$x++;
					if ($x == $cur_img) {
						$img .= '<img src="http://www.france-banderole.com/uploaded/'.$id.'-projects/'.$file.'" />';
					}
				} else if ($x<1) {
					$img .= '<img src="http://www.france-banderole.com/uploaded/'.$id.'-projects/'.$file.'" />';
					$x++;
...
<a href="http://www.france-banderole.com/valider-mon-bat?uid=<?php echo $id; ?>&accepte=1" target="_top" style="margin-left:14px;margin-bottom:10px;width:142px;height:19px;background:url(images/but_bat.png) no-repeat;overflow:hidden;border:none;display:inline-block;margin: auto;"></a>
<?php echo $suivant; ?>
</p>
</body>
</html>